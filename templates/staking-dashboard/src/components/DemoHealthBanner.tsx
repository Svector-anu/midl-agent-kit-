...
const deployEntry = (deploymentLog as { deployments: DeployEntry[] }).deployments.find(
  (d) => d.address.toLowerCase() === STAKING_ADDRESS.toLowerCase(),
);
const contractName = deployEntry?.name ?? "StakingRewards";
const harnessPath = deployEntry?.testHarnessPath ?? "dapps/staking-hardhat";
