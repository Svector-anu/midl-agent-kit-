...
const deployEntry = (deploymentLog as { deployments: DeployEntry[] }).deployments.find(
  (d) => d.address.toLowerCase() === ERC20_TOKEN_ADDRESS.toLowerCase(),
);
const contractName = deployEntry?.name ?? "ERC20Token";
const harnessPath = deployEntry?.testHarnessPath ?? "dapps/erc20-dashboard-hardhat";
